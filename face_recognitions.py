...
for filename in os.listdir(known_folder): # วนลูปอ่าน ชื่อไฟล์ทั้งหมด ในโฟลเดอร์ known_faces
    if filename.endswith(('.jpg', '.png')): #รวจสอบว่าไฟล์นั้นเป็นภาพหรือไม่
        ...
        image = face_recognitions.load_image_file(image_path) #โหลดไฟล์ภาพเข้าเป็นข้อมูลแบบ numpy array
        encoding = face_recognitions.face_encodings(image) # สร้าง เวกเตอร์ใบหน้า (128 ค่า) จากภาพ
...
def face_login(): #ประกาศฟังก์ชัน face_login() — เรียกใช้เมื่อผู้ใช้คลิกปุ่ม "ล็อกอินด้วยใบหน้า"
    ...
    while True: #ลูปอ่านภาพจากกล้องแบบ real-time
        ...
        face_locations = face_recognitions.face_locations(rgb_frame, model='hog') #ตรวจจับตำแหน่งใบหน้า (top, right, bottom, left) โดยใช้โมเดล hog
        face_encodings = face_recognitions.face_encodings(rgb_frame, face_locations) #แปลงแต่ละใบหน้า → เป็นเวกเตอร์ (face encoding ขนาด 128 ค่า)
 
        for face_encoding in face_encodings: #วนลูปทีละใบหน้า (กรณีมีหลายคนในกล้อง)
            matches = face_recognitions.compare_faces(known_face_encodings, face_encoding) #เปรียบเทียบใบหน้าที่เจอ กับฐานข้อมูล → ได้ list ของ True/False
            face_distances = face_recognitions.face_distance(known_face_encodings, face_encoding) #วัดระยะห่าง (คล้ายความแม่นยำ) → ค่าใกล้ 0 = เหมือน
...
def register_user(): # สร้างฟังก์ชันชื่อ register_user()
    ...
    while True: # เริ่มลูปแสดงภาพจากกล้องแบบ real-time
        ...
        if key & 0xFF == ord('s'): #ถ้ากดปุ่ม 's' → เข้าสู่ขั้นตอนบันทึกใบหน้า
            ...
            faces = face_recognitions.face_locations(rgb) #ตรวจจับตำแหน่งใบหน้าในภาพ → ได้ list ของ (top, right, bottom, left)
